...
function App() {
  ...
  return (
    <div style={{width: "100vw", height: "100vh", padding: "16px", overflowY: "scroll", backgroundColor: "lightgray"}}>
      <Grid container direction="row" spacing={3} justifyContent="space-evenly">
        {appCards.map((item: CardInfo) => (
          <Grid item>
            <GameInfoCard heading={item.heading} subHeading={item.subHeading} imgUrl={item.imgUrl} description={item.description} />
          </Grid>
        ))}
      </Grid>
    </div>
